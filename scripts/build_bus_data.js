...
const datasets = [
  ...
  {
    id: 'keisei_chiba_west',
    label: '京成バス千葉ウエスト',
    generatedFrom: 'KeiseiBusChibaWest_AllLines-20260401.zip',
    zipPath: path.join(root, 'gtfs', 'KeiseiBusChibaWest_AllLines-20260401.zip'),
    gtfsDir: path.join(root, '.gtfs_work', 'keisei_chiba_west'),
    mapCenter: [35.6878, 139.9156],
    zoom: 12,
    routePrefix: /\s*$/,
    color: '#1f7a8c',
  ...
  },
  {
    id: 'izuhakone',
    label: '伊豆箱根バス',
    generatedFrom: 'IzuHakone_IZHB-20260701.zip',
    zipPath: path.join(root, 'gtfs', 'IzuHakone_IZHB-20260701.zip'),
    gtfsDir: path.join(root, '.gtfs_work', 'izuhakone'),
    mapCenter: [35.1477, 139.0101],
    zoom: 10,
    routePrefix: /\s*$/,
    color: '#00a3e0',
    useRouteSortKey: true,
  },
  {
    id: 'kyotobus',
    label: '京都バス',
    generatedFrom: 'KyotoBus_AllLinesAnotherversion-20260617.zip',
    zipPath: path.join(root, 'gtfs', 'KyotoBus_AllLinesAnotherversion-20260617.zip'),
    gtfsDir: path.join(root, '.gtfs_work', 'kyotobus'),
    mapCenter: [35.0595, 135.7623],
    zoom: 12,
    routePrefix: /\s*$/,
    color: '#b23a48',
    fallbackLongNameForEmptyShort: true,
...
function routeDisplayName(routeShortName, config, routeLongName = '') {
  const cleanedShortName = (routeShortName || '').replace(config.routePrefix, '').trim();
  if (cleanedShortName) return cleanedShortName;
  if (config.fallbackLongNameForEmptyShort && routeLongName) return routeLongName;
