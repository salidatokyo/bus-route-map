...
GTFS zip を `gtfs` フォルダに置いてからデータを生成します。未展開の場合は `.gtfs_work` 配下に自動展開します。
...
生成結果は軽量な `data.js` と事業者別の `data/*.json` に分割されます。起動時は `data.js` の目録だけを読み、選択された事業者の JSON を必要になったタイミングで取得します。
